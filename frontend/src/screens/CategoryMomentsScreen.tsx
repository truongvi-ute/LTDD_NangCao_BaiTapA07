...
export default function CategoryMomentsScreen() {
  ...
  const loadMoments = async (pageNum: number = 0, append: boolean = false) => {
    try {
      ...
      const response = await momentService.getMomentsByCategoryPaginated(categoryId, pageNum, PAGE_SIZE, sortBy);
      console.log("Loaded category moments page:", pageNum, "count:", response.content.length, "sort:", sortBy);
      
      if (append) {
        setMoments(prev => [...prev, ...response.content]);
      } else {
        setMoments(response.content);
        setTotalMoments(response.totalElements);
      }
  ...
  const loadMore = () => {
    ...
    console.log("Loading more category moments, next page:", page + 1);
    setLoadingMore(true);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerSubtitle}>{totalMoments} khoảnh khắc</Text>
        </View>
          ...
          loading ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtext}>
                Hãy là người đầu tiên chia sẻ khoảnh khắc trong danh mục {categoryName}!
              </Text>
            </View>
          )
        }
